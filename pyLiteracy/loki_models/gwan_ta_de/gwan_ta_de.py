...
import re
try:
    from intent import Loki_gwan_ta_de
except:
    from .intent import Loki_gwan_ta_de
    ...
    if lokiRst.getStatus():
        for index, key in enumerate(inputLIST):
            ...
            for resultIndex in range(0, lokiRst.getLokiLen(index)):
                # gwan_ta_de
                if lokiRst.getIntent(index, resultIndex) == "gwan_ta_de":
                    lokiResultDICT = Loki_gwan_ta_de.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)
...
def testIntent():
    # gwan_ta_de
    print("[TEST] gwan_ta_de")
    inputLIST = ['管他的','他管它的','管它的事','不管他的事','想管它的事']
    testLoki(inputLIST, ['gwan_ta_de'])
    print("")
