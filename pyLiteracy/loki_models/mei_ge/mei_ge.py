...
def testIntent():
    # mei
    print("[TEST] mei")
    inputLIST = ['各人','各個','各所','每人','每天','每次','各一顆','每一顆','各三公尺','各買一各','各買一次','每三公尺','每買一個','每買一次']
    testLoki(inputLIST, ['mei'])
    print("")
...
if __name__ == "__main__":
    ...
    if resultDICT["msg"] == []:
        if "各" in inputSTR:
            print(f"\n在「{inputSTR}」一句中，「各」的用法錯了。")
        elif "每" in inputSTR:
            print(f"\n在「{inputSTR}」一句中，「每」的用法錯了。")
    else:
        print(f"在「{inputSTR}」一句中…")
        print(resultDICT["msg"][0])