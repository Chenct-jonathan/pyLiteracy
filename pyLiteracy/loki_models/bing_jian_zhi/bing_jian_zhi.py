...
import re
try:
    from intent import Loki_human_bing
except:
    from .intent import Loki_human_bing
    ...
    if lokiRst.getStatus():
        for index, key in enumerate(inputLIST):
            ...
            for resultIndex in range(0, lokiRst.getLokiLen(index)):
                # human_bing
                if lokiRst.getIntent(index, resultIndex) == "human_bing":
                    lokiResultDICT = Loki_human_bing.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)
...
def testIntent():
    # human_bing
    print("[TEST] human_bing")
    inputLIST = ['併肩','並肩子']
    testLoki(inputLIST, ['human_bing'])
    print("")
